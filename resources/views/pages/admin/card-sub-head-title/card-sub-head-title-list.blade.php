...
@section('admin.app-content')
    ...
    <div class="row mb-4">
         ...
         <div class="col-sm-6 text-end">
            <a class="btn btn-primary create-btn" href="{{ route('card-sub-head-title.create') }}">Create Card Head Title</a>
         </div>
      </div>
        <div class="card card-box history-table">
            <div class="cardbox-header">
                <h3 class="cardbox-title">Card Sub Head Title List</h3>
                <div class="filter-wrapper">
                    <form action="{{route('card-sub-head-title.index')}}" class="filter-form">
            ...
            <div class="table-responsive">
                <table class="table-borderless trade-table table">
                    ...
                    <tr>
                        ...
                        <th scope="col">Card Head Title</th>
                    ...
                    <tbody>
                        @forelse ($cardSubHeadTitleList as $value)
                            <tr>
                                <td scope="row">{{ $loop->index+1 }}</td>
                                <td scope="row">{{ optional($value->cardHeadTitle)->title }}</td>
                                <td scope="row">{{ $value->title }}</td>
                                <td scope="row">${{ $value->value }}</td>
                                <td scope="row">
                                    <div class="action-cell">
                                        <a href="{{ route('card-sub-head-title.edit', $value->uuid) }}">
      ...
      $(".service-delete-btn").on("click", function() {
         Swal.fire({
            ...
            if (result.isConfirmed) {
               ...
               axios.post(`card-sub-head-title-destroy/${id}`, {
                  "id": id,
                  "_method": "delete"
               }).then(res => {
                  console.log(res)
