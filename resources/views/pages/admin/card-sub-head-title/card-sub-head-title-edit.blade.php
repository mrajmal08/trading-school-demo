...
@section('admin.app-content')
    <div>
        ...
        <form method="post" action="{{ route('card-sub-head-title.update',$cardSubHeadTitle->uuid) }}" enctype='multipart/form-data'>
            @csrf
            <div class="row gy-3 mb-24">
                <div class="col-sm-6">
                    ...
                    <select name="card_head_title_id" class="form-control input-control">
                        ...
                        @foreach($cardHeadTitle as $cardTitle)
                            <option <?= ($cardSubHeadTitle->card_head_title_id == $cardTitle->id)?'selected':'' ?> value="{{ $cardTitle->id }}">{{ $cardTitle->title }}</option>
                        @endforeach
                    </select>
                ...
                <div class="col-sm-6">
                    ...
                    <input class="form-control input-control" type="text" name="title" value="{{ $cardSubHeadTitle->title }}">
                    @if($errors->has('title'))
                ...
                <div class="col-sm-12">
                    ...
                    <input class="form-control input-control" type="text" name="value" value="{{ $cardSubHeadTitle->value }}">
                    @if($errors->has('value'))
