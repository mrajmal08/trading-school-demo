      ...
      <div class="card card-box history-table">
         <div class="cardbox-header">
            ...
            <div class="filter-wrapper">
               <form action="{{ route('account.list') }}" class="filter-form">
                  <div class="row justify-content-end">
                     <div class="col-sm-2">
                        ...
                        <select name="status" id="status" class="select-control dark-th-color" autocomplete="off">
                           ...
                           @foreach ($account_status as $key => $value)
                              <option value="{{ $value }}" {{ $key === $status ? 'selected' : '' }}>
                                 {{ $value }}</option>
                           @endforeach
                        </select>
                     </div>
                     ...
                     <div class="col-sm-2">
                        <input type="submit" class="filter-submit" name="submit" value="Filter">
                     </div>
                  </div>
               </form>
            </div>
                  ...
                  @if (count($allAccounts) > 0)
                     @foreach ($allAccounts as $account)
                        <tr>
                           ...
                           <td scope="row" class="text-end">{{ $account->account_name }}</td>
                           <td scope="row">
                           ...
                           <td scope="row">
                              <div class="action-cell">
                                 @if ($account->account_activation_status == 0)
                                    <a href="{{ route('account.packagePurchaseAccountActivate', $account->uuid) }}" class="process-btn">
                                 ...
                                 @if (empty($account->account_id) || empty($account->trader_id) || empty($account->amp_id))
                                    <a href="{{ route('account.forcedAccount', $account->uuid) }}"
                                       class="force-link-{{ $account->id }} d-none">
                                       <button class="raw-btn edit-btn">
                                          force
                                       </button>
                                    </a>
                                 ...
                                 @if ($account->account_status == 'Success - Pending Verification')
                                    <a href="{{ route('account.activeLiveAccount', $account->uuid) }}">
   ...
   @push('scripts')
      <script>
         $(document).ready(function() {
            ...
            if (localStorage.getItem('admin-process')) {
               $('.' + localStorage.getItem('admin-process')).removeClass('d-none');
            }

            $('.process-btn').on("click", function(e) {
               let siblings = $(this).siblings(".d-none").removeClass('d-none');
               let forceEle = siblings[0].className;
               localStorage.setItem("admin-process", forceEle);
            });
