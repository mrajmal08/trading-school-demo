...
class MarketDataFeedController extends Controller
{
    public function index(Request $request)
    {
        ...
        return view('pages.admin.market-data-feed.market-data-feed-list', compact('marketDataList'));
    }

    public function edit(MarketData $marketData)
    {
        return view('pages.admin.market-data-feed.market-data-feed-edit', compact('marketData'));
    }
    ...
    public function update(Request $request, MarketData $marketData)
    {
        ...
        if ($row) {
            return Redirect::route('market-data-feed.index')->with(['success' => 'Market data feed update successfully']);
        }
    }

    public function subscription(Request $request)
    {
        Log::info(['stripe_cancel' => $request->all()]);
        $stripeConfigKye = config('app.stripe_secret_kye');
        $stripe = new \Stripe\StripeClient($stripeConfigKye);
    ...
    {
        Log::info(['ChargeRefund' => $chargeId]);
        $stripeConfigKye = config('app.stripe_secret_kye');
        ...
        if (!empty($productDetails->metadata['challenge_uuid'])) {
            ...
            if ($userId) {
                ...
                if ($result) {
                    ...
                    $data = array(
                        "traderId" => (string) $userId->traderid,
                    );
                    ...
                    //This code cause Problem The Response is the main cose

                    // $response = Http::post($go_lang_url, [
                    //     "traderId" => $userId->traderid,
                    // ]);
                    Log::info(['RefundJson' => $getData]);
        ...
        //Code to review
            ...
            if ($stripeSystemPaymentUpdate) {
                ...
                Log::info(['MonthlyPay' => $stripeSystemPaymentUpdate]);
    ...
    {
        Log::info(['SubscriptionDelete' => $subscriptionId]);
        $stripeConfigKye = config('app.stripe_secret_kye');
        $stripe = new \Stripe\StripeClient(
            $stripeConfigKye
        );
    ...
    public function findCoupon(Request $request)
    {
        ...
        try {
            ...
            $startDay = !empty($result->metadata->start_date) ? (int)$result->metadata->start_date :  (int)$result->redeem_by;
           
            $currentDay = Carbon::parse(Carbon::now());
            $compareDay = Carbon::parse($startDay);
         
          
            if (($compareDay->greaterThan($currentDay))) {
                return response()->json([
                    'status' => 404,
                    'message' => 'Coupon is not valid for this day',
                ], 401);
            } 
    ...
    public function marketDataPurchaseList(Request $request)
    {
        ...
        return view('pages.admin.market-data-purchase-list', compact(['marketDataDetails', 'userList', 'status', 'search']));
    }
    ...
    public function reProcessMarketData(MarketDataPurchaseDetail $marketDataPurchaseDetail)
    {
        ...
        if ($status === true) {
            ...
            return Redirect::route('marketDataPurchaseList')->with(['success' => 'Market data feed update successfully']);
            ...
            $marketDataPurchaseDetail->log = $response;
            $marketDataPurchaseDetail->save();
            return Redirect::route('marketDataPurchaseList')->with(['error' => 'Market data not updated']);
        }
    }
    ...
    public function testRefund()
    {
        ...
        $customer = $stripe->customers->create([
            'email' => "xarido1125@loongwin.com",
            'name' => "Sylvester Nieves",
            'payment_method' => "pm_1MoXQPG8eX1XeOGiAaVnQKUv",
            'invoice_settings' => ['default_payment_method' => "pm_1MoXQPG8eX1XeOGiAaVnQKUv"],
        ]);
        dd($customer);
        $traderid = "G1156121";
        $go_lang_url = config('app.go_lang_url');
